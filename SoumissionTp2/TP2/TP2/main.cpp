...
int main() {
	...
	*u5 = *u1;
	//1-dereferencement utilsateur 1=utilisateur 2 appel au construvcteur par copie
	//u5 devient u1 meme attribut que u1 puis on chsange le nom
	u5->setNom("Christian");
	...
	//effectuer la surchage des operateurs
	...
	bool terminer = false;
	while (!terminer) {
		cout << "Terminer le programme?" << endl << "O pour oui / N pour non : ";
		cin >> reponse;
		if (reponse == 'O') {
			terminer = true;
		}
