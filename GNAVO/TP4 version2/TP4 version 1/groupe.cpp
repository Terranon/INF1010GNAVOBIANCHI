...
Groupe::~Groupe() {
	//groupe est en composition avec les depenses
...
Groupe& Groupe::operator+=(Utilisateur* utilisateur) {
	//cpmment veroifier que l'utilisateur est ajoutable?
	
	// une autre solution serait de passer par le dynamic cast
	
	if ((dynamic_cast<UtilisateurRegulier*>(utilisateur)))//si l'utilisateur est regulier
  {
	  if ((dynamic_cast<UtilisateurRegulier*>(utilisateur))->getPossedeGroupe() == false)
	  {
		  ...
		  cout << utilisateur->getNom() << " n'est pas un utilisateur premiun et ne peut pas etre ajoute car est deja groupe" << endl;
	...
	//il mettra a jour la methode pour le bon type d'utilisateur..
	
	
	/*
	if(utilisateur->getBalance() != 0 && static_cast<UtilisateurRegulier*>(utilisateur)->getPossedeGroupe()==false){
		utilisateurs_.push_back(utilisateur);
		//allouer l'espace pour le compte de l'utilisateur ajouté
		comptes_.push_back(0);
	}*/
	
	return *this;

	//a chaque fois qu'un utilisateur est ajouté je dois allouer de l'espace pour son compte
}
void Groupe::equilibrerComptes() {
	...
	while (calcul) {
		...
		if (-min <= max && min != 0 && max != 0) {
			...
			//il faut voir le type en regardant ce que en regardant le mode de paiement de l'utilisateur
