...
ajouterDepense de la classe Groupe)*/
	...
	if(estExistant(payePar)){
		//trouve l'iterateur de paye par
	auto payeur = find_if(conteneur_.begin(), conteneur_.end(), bind(equal_to <Utilisateur*>(), _1, payePar));
...
*/
	...
	//finf if retourne l'element pour lequel le predicat est true et renvoie le dernier element du conteneur pour le resultat a false
	auto trouve=find_if(conteneur_.begin(), conteneur_.end(),
	
		bind(equal_to <Utilisateur*>(), _1,utilisateur ));//le premier bind renvoie true si il trouve l'iterateur en question
  */
	
	  //avec un find if simple
	auto trouve=find_if(conteneur_.begin(), conteneur_.end(), bind(equal_to <Utilisateur*>(), _1,utilisateur ) );//le premier bind renvoie true si il trouve l'iterateur en question
...
GestionnaireUtilisateurs& GestionnaireUtilisateurs::setCompte(pair<Utilisateur*, double>p) {
	...
	if (estExistant(p.first)) {
		...
		auto utilisateur=find_if(conteneur_.begin(), conteneur_.end(), bind(equal_to <Utilisateur*>(), _1, p.first));//le premier bind renvoie true si il trouve l'iterateur en question

		//une fois la bonne pair trouvé il faut changer compte
		utilisateur->second = p.second;
